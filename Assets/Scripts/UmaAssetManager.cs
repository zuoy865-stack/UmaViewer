...
{
    if (entry == null) return new List<UmaDatabaseEntry>();

    // 用“可用的 laser 版本”来展开依赖，但仍然保留原 entry（这样加载时还能以原名缓存）
    var depEntry = ResolveLaserCandidateIfMissing(Main, entry);

    if (!string.IsNullOrEmpty(depEntry.Prerequisites))
    {
        ...
        foreach (string prerequisite in depEntry.Prerequisites.Split(';'))
        {
            var key = prerequisite?.Trim();
            if (string.IsNullOrEmpty(key)) continue;

            if (!Main.AbList.TryGetValue(key, out var pre) || pre == null)
            {
                Debug.LogWarning($"[SearchAB] Missing prerequisite '{key}' for '{depEntry.Name}'");
                continue;
            }

            prerequisites.AddRange(SearchAB(Main, pre));
        }

        entry.CachedPrerequisites = prerequisites;
        return new List<UmaDatabaseEntry>() { entry }.Concat(prerequisites).ToList();
    }

    return new List<UmaDatabaseEntry>() { entry };
}
    private static UmaDatabaseEntry ResolveLaserCandidateIfMissing(UmaViewerMain main, UmaDatabaseEntry entry)
{
    ...
    if (main?.AbList == null) return entry;
    ...
    {
        if (main.AbList.TryGetValue(k, out var e) && e != null && File.Exists(e.Path))
        {
            Debug.LogWarning($"[LaserFallback] '{entry.Name}' missing -> use '{k}'");
            return e;
        }
    }
    ...
    public static AssetBundle Get(string name)
    {
        ...
        return instance.LoadedBundles.TryGetValue(name, out var ab) ? ab : null;
    }
    ...
    {
        foreach (var kv in instance.LoadedBundles)
        {
            var ab = kv.Value;
            if (!ab) continue;

            // AB 内资产名往往是完整路径+小写，所以用 endswith 匹配短名
            var hit = ab.GetAllAssetNames()
                .FirstOrDefault(n =>
                    n.EndsWith("/" + shortName, StringComparison.OrdinalIgnoreCase) ||
                    n.EndsWith(shortName, StringComparison.OrdinalIgnoreCase));

            if (!string.IsNullOrEmpty(hit))
                return ab.LoadAsset<Texture2D>(hit);
        }
        return null;
    }
    public static IEnumerable<AssetBundle> EnumerateLoadedBundles()
    {
        ...
        foreach (var ab in instance.LoadedBundles.Values)
        {
            if (ab != null) yield return ab;
        }
    }
}
