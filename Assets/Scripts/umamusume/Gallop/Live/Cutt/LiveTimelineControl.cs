...
namespace Gallop.Live.Cutt
{
    ...
    // 约定：sheet0 负责相机/全局光/背景（你原来就是这么写的）
    ...
    // ===== Camera / Global stuff (只跑 camSheet) =====
    ...
    // ===== Stage controls (对所有 worksheet 都跑) =====
            ...
            private void AlterUpdate_BlinkLight(LiveTimelineWorkSheet workSheet, float currentFrame)
{
    ...
    for (int i = 0; i < list.Count; i++)
    {
        ...
        if (keys.Count <= 0) continue;               // ✅ 注意：Count（不是 count）
        ...
        var curBlink = curKey as LiveTimelineKeyBlinkLightData;  // ✅ 你的 key 类型
        if (curBlink == null) continue;

        BlinkLightUpdateInfo info = default;
        info.data = blinkData;
        info.key = curBlink;
        info.currentFrame = currentFrame;
        info.currentLiveTime = currentLiveTime;
        info.localTime = (currentFrame - curBlink.frame) * kFrameToSec;

        OnUpdateBlinkLight(ref info);
    }
}

private void AlterUpdate_Laser(LiveTimelineWorkSheet workSheet, float currentFrame)
{
    if (OnUpdateLaser == null) return;
    if (workSheet == null || workSheet.laserList == null) return;

    var list = workSheet.laserList;
    if (list.Count <= 0) return;

    for (int i = 0; i < list.Count; i++)
    {
        var laserData = list[i];
        if (laserData == null || laserData.keys == null) continue;

        var keys = laserData.keys;

        // ✅ 这里跟 blinkLight 一样：Count / HasAttribute / EnablePlayModeTimeline 都来自模板基类
        ...
        var curLaser = curKey as LiveTimelineKeyLaserData;
        if (curLaser == null) continue;

        LaserUpdateInfo info = default;
        info.data = laserData;
        info.key = curLaser;
        info.currentFrame = currentFrame;
        info.currentLiveTime = currentLiveTime;
        info.localTime = (currentFrame - curLaser.frame) * kFrameToSec;

        OnUpdateLaser(ref info);
    }
}
