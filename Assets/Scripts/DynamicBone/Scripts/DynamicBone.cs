...
public class DynamicBone : MonoBehaviour
{
    ...
    public bool m_UseSweptCollision = false;
    public bool m_RecheckCollisionAfterLength = false;
    ...
    public class Particle
    {
        ...
        public Vector3 m_AdditionalGravity = Vector3.zero;
        public Vector3 m_AdditionalForce = Vector3.zero;
    ...
    public void UpdateParameters()
    {
        ...
        for (int i = 0; i < Particles.Count; ++i)
        {
            ...
            p.m_Radius = m_Radius;
            p.m_AdditionalGravity = Vector3.zero;
            p.m_AdditionalForce = Vector3.zero;
    ...
    void UpdateParticles1(float timeVar)
    {
        ...
        for (int i = 0; i < Particles.Count; ++i)
        {
            ...
            if (p.m_ParentIndex >= 0)
            {
                ...
                Vector3 particleForce = force + (p.m_AdditionalGravity + p.m_AdditionalForce) * (m_ObjectScale * timeVar);
                p.m_Position += v * (1 - damping) + particleForce + rmove;
            }
            else
            {
    ...
    void UpdateParticles2(float timeVar)
    {
        ...
        for (int i = 1; i < Particles.Count; ++i)
        {
            ...
            if (p.m_Colliders != null)
            {
                ...
                Vector3 collisionReferencePosition = m_UseSweptCollision ? p.m_PrevPosition : p.m_Position;
                for (int j = 0; j < p.m_Colliders.Count; ++j)
                {
                    DynamicBoneColliderBase c = p.m_Colliders[j];
                    if (c != null && c.enabled)
                        p.m_isCollide |= c.Collide(ref p.m_Position, particleRadius, collisionReferencePosition);
                }
            }
            ...
            if (m_RecheckCollisionAfterLength && p.m_Colliders != null)
            {
                float particleRadius = p.m_Radius * m_ObjectScale;
                Vector3 collisionReferencePosition = m_UseSweptCollision ? p.m_PrevPosition : p.m_Position;
                for (int j = 0; j < p.m_Colliders.Count; ++j)
